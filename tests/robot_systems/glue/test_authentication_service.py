"""
Tests for:
  src/engine/auth/authenticated_user.py
  src/engine/auth/authentication_service.py

AuthenticatedUser wraps an AuthUserRecord and satisfies IAuthenticatedUser.
AuthenticationService verifies credentials against IAuthUserRepository.
"""
import unittest
...
def _make_record(user_id: str = "1", password: str = "pass123", role: Role = Role.OPERATOR) -> AuthUserRecord:
    return AuthUserRecord(user_id=user_id, password=password, role=role)


def _repo_with(record: AuthUserRecord | None) -> IAuthUserRepository:
    repo = MagicMock(spec=IAuthUserRepository)
...
class TestAuthenticatedUser(unittest.TestCase):
    ...
    def setUp(self):
        self.record = _make_record(user_id="42", role=Role.ADMIN)
        self.auth_user = AuthenticatedUser(self.record)
    ...
    def test_record_is_accessible(self):
        self.assertIs(self.auth_user.record, self.record)
...
class TestAuthenticate(unittest.TestCase):
    ...
    def test_returns_authenticated_user_on_correct_credentials(self):
        record = _make_record(user_id="1", password="secret")
        svc    = AuthenticationService(_repo_with(record))
    ...
    def test_returns_none_on_wrong_password(self):
        record = _make_record(password="correct")
        svc    = AuthenticationService(_repo_with(record))
...
class TestAuthenticateQr(unittest.TestCase):
    ...
    def test_valid_payload_returns_authenticated_user(self):
        record = _make_record(user_id="1", password="pw")
        svc    = AuthenticationService(_repo_with(record))
