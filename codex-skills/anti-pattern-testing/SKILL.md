...
|---|---|---|
| Does correctness depend on a single function's logic? | Yes | - |
| Does correctness depend on two or more components interacting? | - | Yes |
| Does it involve a real database query, ORM mapping, or transaction? | - | Yes |
| Does it test an HTTP endpoint end-to-end (routing + auth + handler + DB)? | - | Yes |
| Does it verify a queue consumer processes a message correctly? | - | Yes |
| Can all dependencies be replaced with fakes without losing meaning? | Yes | - |
| Has a bug in component interaction burned you before? | - | Yes |
| Does it need to run in under 10ms? | Yes | - |
