from __future__ import annotations

from PyQt6.QtCore import Qt, pyqtSignal
from PyQt6.QtGui import QImage, QPixmap
from PyQt6.QtWidgets import (
    QButtonGroup,
    QCheckBox,
    QComboBox,
    QDoubleSpinBox,
    QFormLayout,
    QGroupBox,
    QHBoxLayout,
    QLabel,
    QLineEdit,
    QPushButton,
    QRadioButton,
    QScrollArea,
    QScrollBar,
    QSizePolicy,
    QSpinBox,
    QTextEdit,
    QVBoxLayout,
    QWidget,
)

import numpy as np

from src.applications.base.i_application_view import IApplicationView
from src.applications.intrinsic_calibration_capture.service.i_intrinsic_capture_service import (
    ARUCO_DICT_OPTIONS,
    IntrinsicCaptureConfig,
)


class IntrinsicCaptureView(IApplicationView):

    start_requested = pyqtSignal()
    stop_requested = pyqtSignal()
    save_config_requested = pyqtSignal(object)  # emits IntrinsicCaptureConfig

    def __init__(self, parent=None):
        super().__init__("Intrinsic Calibration Capture", parent)

    def setup_ui(self) -> None:
        root = QVBoxLayout(self)
        root.setContentsMargins(12, 12, 12, 12)
        root.setSpacing(10)

        # ── Settings ──────────────────────────────────────────────────────────
        settings_box = QGroupBox("Capture Settings")
        form = QFormLayout(settings_box)

        # ── Board type ────────────────────────────────────────────────────────
        board_type_row = QHBoxLayout()
        self._rb_chessboard = QRadioButton("Chessboard")
        self._rb_charuco = QRadioButton("CharuCo")
        self._rb_chessboard.setChecked(True)
        for rb in (self._rb_chessboard, self._rb_charuco):
            rb.setStyleSheet(
                "QRadioButton { color: #e0e0e0; padding: 4px 8px; border-radius: 4px; }"
                "QRadioButton:checked { background: #3a5a8a; color: #ffffff; font-weight: bold; }"
                "QRadioButton::indicator { width: 0px; height: 0px; }"
            )
        self._board_type_group = QButtonGroup(self)
        self._board_type_group.addButton(self._rb_chessboard)
        self._board_type_group.addButton(self._rb_charuco)
        board_type_row.addWidget(self._rb_chessboard)
        board_type_row.addWidget(self._rb_charuco)
        board_type_row.addStretch()
        form.addRow("Board type:", board_type_row)

        # Board parameters (shared)
        self._board_cols = QSpinBox()
        self._board_cols.setRange(0, 50)
        self._board_cols.setValue(0)
        self._board_cols.setSpecialValueText("auto")
        form.addRow("Board cols*:", self._board_cols)

        self._board_rows = QSpinBox()
        self._board_rows.setRange(0, 50)
        self._board_rows.setValue(0)
        self._board_rows.setSpecialValueText("auto")
        form.addRow("Board rows*:", self._board_rows)

        self._square_size = QDoubleSpinBox()
        self._square_size.setRange(0, 200)
        self._square_size.setValue(0.0)
        self._square_size.setSuffix(" mm")
        self._square_size.setSpecialValueText("auto")
        form.addRow("Square size:", self._square_size)

        # CharuCo-specific (hidden when chessboard selected)
        self._charuco_dict_label = QLabel("ArUco dict:")
        self._charuco_dict = QComboBox()
        for name in ARUCO_DICT_OPTIONS:
            self._charuco_dict.addItem(name)
        self._charuco_dict.setCurrentText("4x4_250")
        form.addRow(self._charuco_dict_label, self._charuco_dict)

        self._marker_size_label = QLabel("Marker size:")
        self._marker_size_mm = QDoubleSpinBox()
        self._marker_size_mm.setRange(0, 200)
        self._marker_size_mm.setValue(0.0)
        self._marker_size_mm.setSuffix(" mm")
        self._marker_size_mm.setSpecialValueText("auto (sq × 0.75)")
        form.addRow(self._marker_size_label, self._marker_size_mm)

        # ChArUco sweep parameters (hidden when chessboard selected)
        self._sweep_x_label = QLabel("Sweep X half-range:")
        self._sweep_x_mm = QDoubleSpinBox()
        self._sweep_x_mm.setRange(10, 500)
        self._sweep_x_mm.setValue(100.0)
        self._sweep_x_mm.setSuffix(" mm")
        form.addRow(self._sweep_x_label, self._sweep_x_mm)

        self._sweep_y_label = QLabel("Sweep Y half-range:")
        self._sweep_y_mm = QDoubleSpinBox()
        self._sweep_y_mm.setRange(10, 500)
        self._sweep_y_mm.setValue(100.0)
        self._sweep_y_mm.setSuffix(" mm")
        form.addRow(self._sweep_y_label, self._sweep_y_mm)

        self._min_corners_label = QLabel("Min corners/frame:")
        self._min_corners = QSpinBox()
        self._min_corners.setRange(4, 100)
        self._min_corners.setValue(6)
        form.addRow(self._min_corners_label, self._min_corners)

        self._rz_deg_label = QLabel("RZ yaw variants (±):")
        self._rz_deg = QDoubleSpinBox()
        self._rz_deg.setRange(0, 45)
        self._rz_deg.setValue(15.0)
        self._rz_deg.setSuffix(" °")
        self._rz_deg.setSpecialValueText("disabled")
        form.addRow(self._rz_deg_label, self._rz_deg)

        self._compute_hand_eye_label = QLabel("Auto hand-eye after calib:")
        self._compute_hand_eye = QCheckBox("Enabled")
        self._compute_hand_eye.setChecked(True)
        self._compute_hand_eye.setStyleSheet("QCheckBox { color: #e0e0e0; }")
        form.addRow(self._compute_hand_eye_label, self._compute_hand_eye)

        # Wire board type toggle
        self._rb_chessboard.toggled.connect(self._on_board_type_changed)

        self._grid_rows = QSpinBox()
        self._grid_rows.setRange(1, 10)
        self._grid_rows.setValue(3)
        form.addRow("Grid rows:", self._grid_rows)

        self._grid_cols = QSpinBox()
        self._grid_cols.setRange(1, 10)
        self._grid_cols.setValue(3)
        form.addRow("Grid cols:", self._grid_cols)

        self._margin_label = QLabel("Margin:")
        self._margin_px = QDoubleSpinBox()
        self._margin_px.setRange(0, 200)
        self._margin_px.setValue(60.0)
        self._margin_px.setSuffix(" px")
        form.addRow(self._margin_label, self._margin_px)

        self._tilt_deg = QDoubleSpinBox()
        self._tilt_deg.setRange(0, 30)
        self._tilt_deg.setValue(5.0)
        self._tilt_deg.setSuffix(" °")
        form.addRow("Tilt angle:", self._tilt_deg)

        self._z_delta_mm = QDoubleSpinBox()
        self._z_delta_mm.setRange(0, 200)
        self._z_delta_mm.setValue(40.0)
        self._z_delta_mm.setSuffix(" mm")
        form.addRow("Z delta:", self._z_delta_mm)

        self._probe_dx_label = QLabel("Probe dx:")
        self._probe_dx = QDoubleSpinBox()
        self._probe_dx.setRange(1, 100)
        self._probe_dx.setValue(20.0)
        self._probe_dx.setSuffix(" mm")
        form.addRow(self._probe_dx_label, self._probe_dx)

        self._probe_dy_label = QLabel("Probe dy:")
        self._probe_dy = QDoubleSpinBox()
        self._probe_dy.setRange(1, 100)
        self._probe_dy.setValue(20.0)
        self._probe_dy.setSuffix(" mm")
        form.addRow(self._probe_dy_label, self._probe_dy)

        self._probe_drx_label = QLabel("Probe dRX (roll):")
        self._probe_drx = QDoubleSpinBox()
        self._probe_drx.setRange(0, 30)
        self._probe_drx.setValue(3.0)
        self._probe_drx.setSuffix(" °")
        form.addRow(self._probe_drx_label, self._probe_drx)

        self._probe_dry_label = QLabel("Probe dRY (pitch):")
        self._probe_dry = QDoubleSpinBox()
        self._probe_dry.setRange(0, 30)
        self._probe_dry.setValue(3.0)
        self._probe_dry.setSuffix(" °")
        form.addRow(self._probe_dry_label, self._probe_dry)

        self._probe_drz_label = QLabel("Probe dRZ (yaw):")
        self._probe_drz = QDoubleSpinBox()
        self._probe_drz.setRange(0, 30)
        self._probe_drz.setValue(0.0)
        self._probe_drz.setSuffix(" °")
        form.addRow(self._probe_drz_label, self._probe_drz)

        self._on_board_type_changed(True)  # apply initial visibility

        self._stab_delay = QDoubleSpinBox()
        self._stab_delay.setRange(0.0, 5.0)
        self._stab_delay.setSingleStep(0.1)
        self._stab_delay.setValue(0.5)
        self._stab_delay.setSuffix(" s")
        form.addRow("Stabilization delay:", self._stab_delay)

        self._max_detection_retries = QSpinBox()
        self._max_detection_retries.setRange(1, 100)
        self._max_detection_retries.setValue(1)
        form.addRow("Detection retries:", self._max_detection_retries)

        self._initial_detection_attempts = QSpinBox()
        self._initial_detection_attempts.setRange(1, 30)
        self._initial_detection_attempts.setValue(5)
        form.addRow("Initial detect attempts:", self._initial_detection_attempts)

        self._initial_detection_delay = QDoubleSpinBox()
        self._initial_detection_delay.setRange(0.1, 10.0)
        self._initial_detection_delay.setSingleStep(0.5)
        self._initial_detection_delay.setValue(1.0)
        self._initial_detection_delay.setSuffix(" s")
        form.addRow("Initial detect delay:", self._initial_detection_delay)

        self._velocity = QSpinBox()
        self._velocity.setRange(1, 100)
        self._velocity.setValue(20)
        self._velocity.setSuffix(" %")
        form.addRow("Velocity:", self._velocity)

        self._acceleration = QSpinBox()
        self._acceleration.setRange(1, 100)
        self._acceleration.setValue(10)
        self._acceleration.setSuffix(" %")
        form.addRow("Acceleration:", self._acceleration)

        self._output_dir = QLineEdit()
        self._output_dir.setPlaceholderText("Leave empty for ./intrinsic_capture_output")
        form.addRow("Output dir:", self._output_dir)

        scroll = QScrollArea()
        scroll.setWidgetResizable(True)
        scroll.setWidget(settings_box)
        scroll.setMinimumHeight(300)
        root.addWidget(scroll, stretch=1)

        # ── Buttons ───────────────────────────────────────────────────────────
        btn_row = QHBoxLayout()
        self._start_btn = QPushButton("Start Capture")
        self._start_btn.setFixedHeight(36)
        self._stop_btn = QPushButton("Stop")
        self._stop_btn.setFixedHeight(36)
        self._stop_btn.setEnabled(False)
        btn_row.addWidget(self._start_btn)
        btn_row.addWidget(self._stop_btn)
        root.addLayout(btn_row)

        # ── Log ───────────────────────────────────────────────────────────────
        log_box = QGroupBox("Log")
        log_layout = QVBoxLayout(log_box)
        self._log = QTextEdit()
        self._log.setReadOnly(True)
        self._log.setMinimumHeight(180)
        log_layout.addWidget(self._log)
        root.addWidget(log_box)

        # ── Frame preview ─────────────────────────────────────────────────────
        self._preview = QLabel("No frame")
        self._preview.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self._preview.setMinimumHeight(160)
        self._preview.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding)
        root.addWidget(self._preview)

        # ── Wire internal signals ─────────────────────────────────────────────
        self._start_btn.clicked.connect(self._on_start_clicked)
        self._stop_btn.clicked.connect(self.stop_requested)

    def clean_up(self) -> None:
        pass

    # ── Accessors ─────────────────────────────────────────────────────────────

    def get_config(self) -> IntrinsicCaptureConfig:
        return IntrinsicCaptureConfig(
            board_type="charuco" if self._rb_charuco.isChecked() else "chessboard",
            chessboard_width=self._board_cols.value(),
            chessboard_height=self._board_rows.value(),
            square_size_mm=self._square_size.value(),
            aruco_dict=self._charuco_dict.currentText(),
            marker_size_mm=self._marker_size_mm.value(),
            grid_rows=self._grid_rows.value(),
            grid_cols=self._grid_cols.value(),
            margin_px=self._margin_px.value(),
            tilt_deg=self._tilt_deg.value(),
            z_delta_mm=self._z_delta_mm.value(),
            probe_dx_mm=self._probe_dx.value(),
            probe_dy_mm=self._probe_dy.value(),
            probe_drx_deg=self._probe_drx.value(),
            probe_dry_deg=self._probe_dry.value(),
            probe_drz_deg=self._probe_drz.value(),
            stabilization_delay_s=self._stab_delay.value(),
            max_detection_retries=self._max_detection_retries.value(),
            initial_detection_attempts=self._initial_detection_attempts.value(),
            initial_detection_delay_s=self._initial_detection_delay.value(),
            velocity=self._velocity.value(),
            acceleration=self._acceleration.value(),
            output_dir=self._output_dir.text().strip(),
            charuco_sweep_x_mm=self._sweep_x_mm.value(),
            charuco_sweep_y_mm=self._sweep_y_mm.value(),
            charuco_min_corners=self._min_corners.value(),
            charuco_rz_deg=self._rz_deg.value(),
            charuco_compute_hand_eye=self._compute_hand_eye.isChecked(),
        )

    def set_config(self, config: IntrinsicCaptureConfig) -> None:
        self._rb_charuco.setChecked(config.board_type == "charuco")
        self._rb_chessboard.setChecked(config.board_type != "charuco")
        self._board_cols.setValue(config.chessboard_width)
        self._board_rows.setValue(config.chessboard_height)
        self._square_size.setValue(config.square_size_mm)
        self._charuco_dict.setCurrentText(config.aruco_dict)
        self._marker_size_mm.setValue(config.marker_size_mm)
        self._grid_rows.setValue(config.grid_rows)
        self._grid_cols.setValue(config.grid_cols)
        self._margin_px.setValue(config.margin_px)
        self._tilt_deg.setValue(config.tilt_deg)
        self._z_delta_mm.setValue(config.z_delta_mm)
        self._probe_dx.setValue(config.probe_dx_mm)
        self._probe_dy.setValue(config.probe_dy_mm)
        self._probe_drx.setValue(config.probe_drx_deg)
        self._probe_dry.setValue(config.probe_dry_deg)
        self._probe_drz.setValue(config.probe_drz_deg)
        self._stab_delay.setValue(config.stabilization_delay_s)
        self._max_detection_retries.setValue(config.max_detection_retries)
        self._initial_detection_attempts.setValue(config.initial_detection_attempts)
        self._initial_detection_delay.setValue(config.initial_detection_delay_s)
        self._velocity.setValue(config.velocity)
        self._acceleration.setValue(config.acceleration)
        self._output_dir.setText(config.output_dir)
        self._sweep_x_mm.setValue(config.charuco_sweep_x_mm)
        self._sweep_y_mm.setValue(config.charuco_sweep_y_mm)
        self._min_corners.setValue(config.charuco_min_corners)
        self._rz_deg.setValue(config.charuco_rz_deg)
        self._compute_hand_eye.setChecked(config.charuco_compute_hand_eye)

    def set_running(self, running: bool) -> None:
        self._start_btn.setEnabled(not running)
        self._stop_btn.setEnabled(running)

    def append_log(self, message: str) -> None:
        self._log.append(message)
        sb = self._log.verticalScrollBar()
        sb.setValue(sb.maximum())

    def set_frame(self, frame: np.ndarray) -> None:
        if frame is None:
            return
        h, w = frame.shape[:2]
        rgb = frame[:, :, ::-1].copy()  # BGR → RGB
        qimg = QImage(rgb.data, w, h, w * 3, QImage.Format.Format_RGB888)
        pix = QPixmap.fromImage(qimg).scaled(
            self._preview.width(), self._preview.height(),
            Qt.AspectRatioMode.KeepAspectRatio,
            Qt.TransformationMode.SmoothTransformation,
        )
        self._preview.setPixmap(pix)

    # ── Internal ──────────────────────────────────────────────────────────────

    def _on_board_type_changed(self, chessboard_selected: bool) -> None:
        charuco = not chessboard_selected
        # CharuCo-only widgets
        for w in (self._charuco_dict_label, self._charuco_dict,
                  self._marker_size_label, self._marker_size_mm,
                  self._sweep_x_label, self._sweep_x_mm,
                  self._sweep_y_label, self._sweep_y_mm,
                  self._min_corners_label, self._min_corners,
                  self._rz_deg_label, self._rz_deg,
                  self._compute_hand_eye_label, self._compute_hand_eye):
            w.setVisible(charuco)
        # Chessboard-only widgets (Jacobian probe + margin)
        for w in (self._margin_label, self._margin_px,
                  self._probe_dx_label, self._probe_dx,
                  self._probe_dy_label, self._probe_dy,
                  self._probe_drx_label, self._probe_drx,
                  self._probe_dry_label, self._probe_dry,
                  self._probe_drz_label, self._probe_drz):
            w.setVisible(chessboard_selected)

    def _on_start_clicked(self) -> None:
        self.save_config_requested.emit(self.get_config())
        self.start_requested.emit()
