...
class _BaseCalibrationTab(QWidget):
    def __init__(self, title: str, hint: str, parent=None):
        ...
        # Inner card holds all content
        self._card = QWidget()
        self._card.setStyleSheet(APP_CARD_STYLE)
        self._card_layout = QVBoxLayout(self._card)
        self._card_layout.setContentsMargins(16, 14, 16, 16)
        ...
        # No addStretch — content sits at top, scroll handles overflow

        # Each tab scrolls independently — tab bar stays pinned in the panel
        scroll = QScrollArea()
        scroll.setWidgetResizable(True)
        scroll.setFrameShape(QScrollArea.Shape.NoFrame)
        scroll.setHorizontalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
        scroll.setStyleSheet(f"background: {APP_BG};")
...
class CameraCalibrationTab(_BaseCalibrationTab):
    def __init__(
        ...
        overlay_row.addWidget(crosshair_btn, stretch=1)
        overlay_row.addWidget(magnifier_btn, stretch=1)
