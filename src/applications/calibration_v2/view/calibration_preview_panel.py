...
class CalibrationAreaGridPanel(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        area_layout.setContentsMargins(16, 14, 16, 16)
        ...
        # ── Work-area selector (full width) ──────────────────────────────────
        area_layout.addWidget(QLabel("Work area:"))
        self.work_area_combo = QComboBox()
        self.work_area_combo.setMinimumHeight(40)
        for definition in self._work_area_definitions:
            self.work_area_combo.addItem(definition.label, definition.id)
        area_layout.addWidget(self.work_area_combo)

        # ── Grid shape: Rows | Cols  (two equal columns) ──────────────────────
        grid_shape_row = QHBoxLayout()
        grid_shape_row.setSpacing(10)
        ...
        rows_col.addWidget(self.grid_rows_spin)
        grid_shape_row.addLayout(rows_col, stretch=1)
        ...
        cols_col.addWidget(self.grid_cols_spin)
        grid_shape_row.addLayout(cols_col, stretch=1)

        area_layout.addLayout(grid_shape_row)
        area_layout.addWidget(divider())
        ...
        area_layout.addWidget(self.generate_area_grid_btn)
        ...
        area_layout.addWidget(self.measure_area_grid_btn)
        ...
        area_layout.addWidget(self.verify_area_grid_btn)

        # ── Secondary row: Clear | Depth Map ─────────────────────────────────
        ...
        secondary_row.addWidget(self.clear_area_grid_btn, stretch=1)

        self.view_depth_map_btn = MaterialButton("📈  Depth Map")
        self.view_depth_map_btn.setStyleSheet(APP_PRIMARY_BUTTON_STYLE)
        self.view_depth_map_btn.setEnabled(False)
        secondary_row.addWidget(self.view_depth_map_btn, stretch=1)
        area_layout.addLayout(secondary_row)
...
class CalibrationPreviewPanel(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        # ── Collapsible log ───────────────────────────────────────────────────
        from pl_gui.utils.utils_widgets.MaterialButton import MaterialButton as _Btn
        from src.applications.base.app_styles import APP_SECONDARY_BUTTON_STYLE

        self._log_toggle_btn = _Btn("▼  Activity Log")
        self._log_toggle_btn.setStyleSheet(APP_SECONDARY_BUTTON_STYLE)
        ...
        log_body_layout.setContentsMargins(0, 4, 0, 0)
        ...
        self._log_body.hide()   # collapsed by default — camera gets full height

        layout.addWidget(preview_card, stretch=1)
        layout.addWidget(self._log_toggle_btn, stretch=0)
        layout.addWidget(self._log_body, stretch=0)

    def _toggle_log(self) -> None:
        visible = not self._log_body.isVisible()
        self._log_body.setVisible(visible)
        self._log_toggle_btn.setText("▲  Activity Log" if visible else "▼  Activity Log")
