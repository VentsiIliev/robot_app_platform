...
from editor_core.ui.CreateWorkpieceForm import (
...
class FormFieldConfig:
    ...
    field_type:    str
    label:         str
    icon:          Any = ""     # FieldIcon | str (legacy file path) | ""
    visible:       bool = True
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'FormFieldConfig':
        return cls(
            ...
            label=data["label"],
            icon=data.get("icon", data.get("icon_path", "")),
...
class CreateWorkpieceForm(Drawer, QFrame):
    ...
    def create_icon_label(self, icon, size=50):
        ...
        pixmap = QPixmap()  # null by default

        if isinstance(icon, FieldIcon):
            if icon.kind == "path":
                if icon.value and os.path.exists(icon.value):
                    pixmap = QPixmap(icon.value)
                else:
                    _log.warning("FieldIcon path not found: %r", icon.value)
            elif icon.kind == "qta":
                try:
                    import qtawesome as qta
                    pixmap = qta.icon(icon.value, color=icon.color).pixmap(size, size)
                except Exception as exc:
                    _log.warning("FieldIcon qta failed for %r: %s", icon.value, exc)
            # kind == "none" → leave as null pixmap (transparent placeholder)
    ...
    def resizeEvent(self, event):
        ...
        if hasattr(self, 'submit_button') and self.submit_button:
            self.submit_button.setIconSize(QSize(int(newWidth * 0.05), int(newWidth * 0.05)))
        if hasattr(self, 'cancel_button') and self.cancel_button:
            self.cancel_button.setIconSize(QSize(int(newWidth * 0.05), int(newWidth * 0.05)))
