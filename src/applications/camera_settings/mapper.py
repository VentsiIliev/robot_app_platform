...
class CameraSettingsMapper:
    ...
    def to_flat_dict(data: CameraSettingsData) -> dict:
        ...
        d.pop("brightness_area_points", None)
        return d

    @staticmethod
    def from_flat_dict(flat: dict, base: CameraSettingsData) -> CameraSettingsData:
        updates = {}
        for f in dataclasses.fields(CameraSettingsData):
            if f.name not in flat or flat[f.name] is None:
                continue
            v = flat[f.name]
            if f.default is dataclasses.MISSING:
                updates[f.name] = v
                continue
            target_type = type(f.default)
            try:
                updates[f.name] = target_type(v)
