...
class ApplicationManager(IApplicationManager):
    ...
    def state(self) -> ApplicationBusyState:
        return ApplicationBusyState.BUSY if self._active else ApplicationBusyState.IDLE

    def acquire(self, process_id: str) -> bool:
        with self._lock:
            if self._active is None or self._active == process_id:
                self._active = process_id
                self._publish(ApplicationBusyState.BUSY, process_id)
    ...
    def release(self, process_id: str) -> None:
        with self._lock:
            if self._active != process_id:
                return
            self._active = None
            self._publish(ApplicationBusyState.IDLE, None)
            self._logger.info("Released by '%s'", process_id)

    def _publish(self, state: ApplicationBusyState, active: Optional[str]) -> None:
        event = ApplicationStateEvent(state=state, active_process=active)
        self._messaging.publish(ApplicationTopics.STATE, event)