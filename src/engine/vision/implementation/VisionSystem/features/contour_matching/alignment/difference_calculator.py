...
def _ensure_closed(contour: Contour) -> Contour:
    ...
    if not np.array_equal(points[0], points[-1]):
        closed_points = np.vstack([points, points[0].reshape(1, 2)])
...
def _debug_contours(workpieceContour, contour, wpAngle, contourAngle, rotationDiff,
    ...
    # Optional debug plot
    from src.engine.vision.implementation.VisionSystem.features.contour_matching.debug.plot_generator import get_similarity_debug_plot
