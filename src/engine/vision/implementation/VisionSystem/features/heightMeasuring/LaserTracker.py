...
class LaserTrackService:
    ...
    def measure_height(self, image):
        ...
        try:
            ...
            import traceback
            traceback.print_exc()
            _logger.error(f"LaserTrackService measurement error: {e}")
...
class LaserTracker(object):
    ...
    def save_calibration_data(self):
        ...
        _logger.info(f"✅ Calibration data saved to {self.save_file}")
        ...
        if os.path.exists(self.save_file):
            ...
            if data.get("axis") == self.axis:
                ...
                if poly_coeffs:
                    ...
                    _logger.info("✅ Calibration model loaded from JSON.")

                _logger.info(f"✅ Loaded calibration data: zero reference={self.reference_point}, "

                      f"{len(self.calibration_points)} calibration points")
            else:
                _logger.warning(f"⚠️ Saved calibration data axis mismatch (saved: {data.get('axis')}, current: {self.axis})")

        else:
            _logger.info("ℹ️ No saved calibration data found — you may need to calibrate.")
    ...
    def calibrate_zero_height(self):
        if self.previous_position:
            ...
            _logger.info(f"Zero reference set for axis {self.axis.upper()} at {pixel_value} pixels")

            self.save_calibration_data()
            return pixel_value
        else:
            _logger.info("No laser detected! Cannot calibrate.")
    ...
    def add_calibration_point(self, pixel_diff, real_height):
        ...
        self.save_calibration_data()
        _logger.info(f"Added calibration point: pixel_diff={pixel_diff}, real_height={real_height} mm")
...
if __name__ == '__main__':
    ...
    configFilePath = "/home/plp/cobot-soft-v2/cobot-glue-dispencing-v2/cobot-soft-glue-dispencing-v2/vision_service/storage/settings/camera_settings.json"
    ...
    while True:
        ...
        if frame is None:
            _logger.info("No frame captured from camera.")
        ...
        if key == ord('q'):
            ...
            _logger.info("Zero reference set!")
            ...
            if diff is not None:
                ...
                tracker.save_calibration_data()
                _logger.info(f"Collected calibration point: pixel_diff={diff}, height={real_height} mm")
            ...
            if len(calibration_points) >= 2:
                ...
                tracker.calibration_points = calibration_points
                tracker.save_calibration_data()
                _logger.info("New calibration model saved!")

            else:
                _logger.info("Need at least 2 calibration points to fit a model.")
            ...
            _logger.info(f"Measured Diff: {diff} px, Max Displacement: {max_disp} px at {max_point}")
