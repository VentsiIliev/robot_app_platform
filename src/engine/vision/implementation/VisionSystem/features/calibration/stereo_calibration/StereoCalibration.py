...
_logger.info(f"Left images path: {left_images_path}")

_logger.info(f"Right images path: {right_images_path}")
...
_logger.info(f"[INFO] Found {len(left_images)} image pairs for calibration...")


for i, (left_path, right_path) in enumerate(zip(left_images, right_images)):
    img_left = cv2.imread(left_path)
    img_right = cv2.imread(right_path)
    _logger.info(f"[INFO] Processing pair {i + 1}:")

    _logger.info(f"       Left: {left_path}")

    _logger.info(f"       Right: {right_path}")
...
_logger.info(f"[INFO] Successfully detected corners in {len(objpoints)} pairs.")


if len(objpoints) == 0:
    _logger.info("No corners were detected in any image pair. Check your images and chessboard parameters.")
...
_logger.info(f"Baseline (distance between cameras): {baseline:.2f} units")

_logger.info("[INFO] Stereo calibration completed.")

_logger.info("Rotation Matrix:\n", R)

_logger.info("Translation Vector:\n", T)
...
_logger.info("[INFO] Computing rectification transforms and remapping images safely...")
...
if test_left.shape[:2][::-1] != gray_left.shape[::-1]:
    _logger.warning("[WARNING] Test image size does not match calibration image size!")
...
_logger.info("[INFO] Rectified test pair saved successfully!")

_logger.info(f"Rectified image shapes: Left={rect_left.shape}, Right={rect_right.shape}")
...
_logger.info("[INFO] Computing disparity map...")
...
_logger.info("[INFO] Disparity map computed and saved.")
...
_logger.info("[INFO] Reprojecting disparity to 3D...")
...
_logger.info(f"[INFO] 3D point cloud saved to {output_dir}/point_cloud.ply")


_logger.info("[✅ DONE] Stereo calibration and depth estimation pipeline complete!")
