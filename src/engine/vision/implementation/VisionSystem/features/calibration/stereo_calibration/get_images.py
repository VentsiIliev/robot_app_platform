...
def get_image(url):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            _logger.error(f"[WARN] Failed to get image from {url}, status: {response.status_code}")

            return None
    except requests.exceptions.RequestException as e:
        _logger.error(f"[ERROR] Could not connect to {url}: {e}")
...
# -------------------------------------------------------------
img_counter = 0
_logger.info("[INFO] Press 's' to save stereo image pair | Press 'q' to quit")
...
while True:
    ...
    if left_img is None or right_img is None:
        _logger.warning("[WARN] Could not retrieve images. Retrying...")
    ...
    if key == ord('s'):
        ...
        img_counter += 1
        _logger.info(f"[INFO] Saved image pair #{img_counter}:")

        _logger.info(f"   {left_path}")

        _logger.info(f"   {right_path}")


    # Quit
    elif key == ord('q'):
        _logger.info("[INFO] Exiting capture loop.")
