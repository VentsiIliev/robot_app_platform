...
class PaintWorkpiecePathExecutor(IWorkpiecePathExecutor):
    ...
    def execute_pickup_and_paint(
        ...
        with timing_session("pickup_and_paint") as recorder:
            ...
            if not ok:
                _logger.info("[TIMING] pickup_and_paint success=false stage=pickup total_elapsed_s=%.3f", elapsed_s(started))
                result = (False, msg)
            else:
    ...
    def _run_post_execute_return_after_pickup(
        ...
        _logger.info("[TIMING] pickup_and_paint success=false stage=post_return total_elapsed_s=%.3f", elapsed_s(started))
        if result[0]:
            return False, msg
