...
class GlueWorkpieceEditorAdapter(IWorkpieceDataAdapter):
    """Glue-specific editor adapter.

    Glue continues to own the ``sprayPattern`` mapping used by its persisted
    workpiece payloads. The generic editor only sees an injected adapter.
    """
    ...
    def to_workpiece_data(
        ...
        _logger.debug(
            "glue to_workpiece_data — Workpiece/Main segs=%d Contour segs=%d Fill segs=%d",
    ...
    def from_raw(self, raw: dict) -> ContourEditorData:
        ...
        layer_data = {
            workpiece_layer_name(): [{"contour": raw_contour, "settings": {}}],
