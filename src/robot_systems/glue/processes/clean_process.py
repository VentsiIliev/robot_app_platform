...
class CleanProcess(BaseProcess):
    ...
    def _on_start(self) -> None:
        self._logger.info("Clean started — simulating %.1fs of work", self._simulation_duration)
    ...
    def _on_stop(self) -> None:
        self._cancel_timer()
        self._logger.info("Clean stopped")

    def _on_pause(self) -> None:
        self._cancel_timer()
        self._logger.info("Clean paused")

    def _on_resume(self) -> None:
        self._logger.info("Clean resumed — simulating %.1fs remaining", self._simulation_duration)
        self._sim_timer = threading.Timer(self._simulation_duration, self._simulation_complete)
        self._sim_timer.daemon = True
        self._sim_timer.start()
    ...
    def _simulation_complete(self) -> None:
        self._logger.info("Clean simulation complete — stopping")
        self.stop()
