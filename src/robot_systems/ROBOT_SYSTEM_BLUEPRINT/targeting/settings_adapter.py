...
def to_editor_dict(settings: MyTargetingSettings | None) -> dict:
    targeting = deepcopy(settings) if settings is not None else MyTargetingSettings.defaults()
    ...
    return {
        ...
        "protected_points": [CAMERA_POINT],
        "protected_frames": [CALIBRATION_FRAME],
    }


def from_editor_dict(data: dict, base: MyTargetingSettings | None) -> MyTargetingSettings:
    points = [
        MyTargetPoint(
    ...
    settings = deepcopy(base) if base is not None else MyTargetingSettings.defaults()
