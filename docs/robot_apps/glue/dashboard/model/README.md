# `src/robot_apps/glue/dashboard/model/` — Glue Dashboard Model
...
- **`IPluginModel` for interface uniformity**: The dashboard uses `IPluginModel` as its base even though it doesn't persist anything. This ensures the controller can call `model.load()` in the standard lifecycle pattern.
