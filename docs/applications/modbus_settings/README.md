# `src/applications/modbus_settings/` — Modbus Settings Application

The `modbus_settings` application provides a GUI for configuring the Modbus RTU serial connection (port, baud rate, etc.), detecting available serial ports, and testing whether the current configuration can connect successfully. It follows the standard application MVC pattern with an additional `IModbusActionService` for hardware actions that are separate from settings persistence.
...
```
ModbusSettingsApplication(IApplication)
  └─ ModbusSettingsFactory(ApplicationFactory).build(settings_service, action_service)
...
| Class | Role |
|-------|------|
| `ModbusSettingsApplication(IApplication)` | Bootstrap entry point; constructs both services and factory |
| `ModbusSettingsFactory(ApplicationFactory)` | Overrides `build()` to accept two services |
| `IModbusSettingsService` | ABC: `load_config() → ModbusConfig`, `save_config(config)` |
| `ModbusSettingsApplicationService` | Wraps `ISettingsService`; key `"modbus_config"` |
...
- **`ModbusSettingsFactory` overrides `build()`**: The base `ApplicationFactory.build()` accepts `*args`. `ModbusSettingsFactory` unpacks two positional services and passes them through to `_create_model`.
