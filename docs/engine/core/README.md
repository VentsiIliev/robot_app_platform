...
| `transform_to_tcp(x, y)` | Same as `transform`, then adds camera-to-TCP offset. Raises `RuntimeError` if matrix not loaded **or** if no TCP offsets were provided at construction. |
| `transform_to_tool(x, y)` | Same as `transform`, then adds camera-to-tool offset. Raises `RuntimeError` if matrix not loaded **or** if no tool offsets were provided at construction. |
