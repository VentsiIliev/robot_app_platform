# `src/plugins/robot_settings/` — Robot Settings Plugin

The `robot_settings` plugin provides a GUI for editing all robot configuration: network settings, global motion parameters, TCP offsets, safety limits, offset direction map, movement groups (named positions), and calibration parameters. It follows the standard plugin MVC pattern.
...
```
RobotSettingsPlugin(IPlugin)
  └─ RobotSettingsFactory(PluginFactory).build(settings_service)
...
| Class | Role |
|-------|------|
| `RobotSettingsPlugin(IPlugin)` | Bootstrap entry point; constructs `RobotSettingsPluginService` + factory |
| `RobotSettingsFactory(PluginFactory)` | Standard 3-method override |
| `IRobotSettingsService` | ABC: load/save `RobotSettings` + `RobotCalibrationSettings` |
| `RobotSettingsPluginService` | Wraps `ISettingsService`; keys `"robot_config"` + `"robot_calibration"` |
...
- **No broker subscriptions**: `RobotSettingsController.stop()` is a no-op. The plugin does not receive live data.
